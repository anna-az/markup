const openReserModel = document.querySelectorAll('.reservation');
openReserModel.forEach(element => element.addEventListener('click', openReservationModel));

const submitForm = document.querySelector('#reservation-none-model');
...
function openReservationModel(event) {
    ...
    const form = document.querySelector('#review-form-reserv');

    const closeReservModal = document.querySelector('#close_reserv_modal');
    closeReservModal.addEventListener('click', function(event) {
        modal_container.innerHTML=" ";
    });

    const submit = document.querySelector('#reserv_form_model');
...
function sendReservData(event) {
    ...
    */
    ...
    modal_container.innerHTML=" ";
    event.target.reset();
    const text = 'Мы свяжемся с вами в ближайшее время.';
